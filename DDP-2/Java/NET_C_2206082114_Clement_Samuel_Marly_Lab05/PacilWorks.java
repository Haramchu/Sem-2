...
public class PacilWorks {
    ...
    private static void handleAddEmployee(){
        ...
        if(role.equalsIgnoreCase("Engineer")) {
            // minta input dan masuk ke dalam class enginner serta array employee
            System.out.print("Nama: ");
            String nama = in.nextLine();
            System.out.print("Banyak Side Jobs: ");
            String inputSideJobs = in.nextLine();
            int sideJobs = Integer.parseInt(inputSideJobs);
            Engineer add = new Engineer(nama, sideJobs);
            employees.add(add);

        } else if(role.equalsIgnoreCase("Secretary")) {
            // minta input dan masuk ke dalam class secretary serta array employee
            System.out.print("Nama: ");
            String nama = in.nextLine();
            System.out.print("Banyak Tunjangan: ");
            String inputTunjangan = in.nextLine();
            Double tunjangan = Double.parseDouble(inputTunjangan);
            Secretary add = new Secretary(nama, tunjangan);
            employees.add(add);

        } else if(role.equalsIgnoreCase("Manager")) {
            // minta input dan masuk ke dalam class manager serta array employee
            System.out.print("Nama: ");
            String nama = in.nextLine();
            Manager add = new Manager(nama, 1.25);
            add.setGaji(2000000.0);
            employees.add(add);
